...
1340 ;
1346 ; -----------
1348 ; GETFXD()/GETINT() -> A, X
1349 ; GETFXD: get line # from input, (GETINT) return binary int in A,X
1350 GETFXD JSR INPFXD
1352 GETINT LDA INTVAL
1360 LDX INTVAL+1
1362 RTS
1364 ; -----------
1370 ;
...
1610 RTS
1619 ; XXX should be no need for this, check params before mult
1620 MULTERR LDA #$FF
...
1800 LDY #0
1802 LDA (GETPTR),Y
1804 TAX; XX no need?: LDA,STA; LDA,STA
...
2048 ; XXX take out protection of Wedge memory
2050 INC BLOCK
...
2160 LDA (STRPTR),Y
2170 STY TEMPH:JSR PRINTBYT:LDY TEMPH; XX no need to save Y?
...
3210 CNTON LDY #0
3220 LDA (CNTPTR),Y
3222 TAX; XX Save this, simply LDA,STA; LDA,STA
...
4450 ;
4458 ; XX NOTE: put these (and others?) under ROM
4460 FILENAME *= *+55
4470 TOKNUM *=*+1
4480 TEMPX *=*+1
4490 TEMPY *=*+1
